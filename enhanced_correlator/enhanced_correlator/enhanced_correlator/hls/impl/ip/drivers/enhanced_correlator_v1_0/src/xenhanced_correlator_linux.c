...
typedef struct {
    ...
    XEnhanced_correlator_uio_map maps[ MAX_UIO_MAPS ];
} XEnhanced_correlator_uio_info;

/***************** Variable Definitions **************************************/
static XEnhanced_correlator_uio_info uio_info;
...
static int uio_info_read_name(XEnhanced_correlator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XEnhanced_correlator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XEnhanced_correlator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XEnhanced_correlator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XEnhanced_correlator_Initialize(XEnhanced_correlator *InstancePtr, const char* InstanceName) {
	XEnhanced_correlator_uio_info *InfoPtr = &uio_info;
